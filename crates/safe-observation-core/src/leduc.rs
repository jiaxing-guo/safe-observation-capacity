//! Leduc algorithms for safe observation. See supplementary Reproducibility for its role in the release workflow.

use crate::game::{Game, Node};

#[derive(Clone, Copy)]
/// Stores state for Leduc rules.
...
/// Implements operations for `LeducRules`.
impl LeducRules {
    /// Computes standard.
...
/// Stores state for Leduc.
pub struct Leduc;

/// Stores state for Leduc big.
pub struct LeducBig;
...
/// Implements operations for `Leduc`.
impl Game for Leduc {
...
/// Implements operations for `LeducBig`.
...
mod tests {
    ...
    /// Defines the Leduc value constant.
    const LEDUC_VALUE: f64 = -0.0856064240780;

    #[test]
    /// Verifies that winner pairs and ranks.
    ...
    #[test]
    /// Verifies that sequence form sizes match literature.
    ...
    #[test]
    /// Verifies that blueprint value matches literature.
